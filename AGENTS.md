...
| Hermes AI crew (systemd `regime-locus-crew`) | `pip install -e ".[hermes]"` then `python3 scripts/run_crew.py`. Uses `RLM_HERMES_BASE_URL` (default `http://127.0.0.1:11434/v1`), `RLM_HERMES_MODEL`, optional `RLM_HERMES_SKIP_MEMORY=1`. RLM tools register as Hermes toolset `rlm`. Skill templates: `hermes_skills/`. **This is the production Scotty / Spock / Kirk loop** (engineering report → research → commander). |
| Host watchdog (`scotty.service` on some VPSes) | Lightweight process from `scripts/rlm_enterprise_watchdog.py` (installed by `bootstrap.sh` as `/opt/enterprise/agents/scotty.py`): polls systemd + resources, optional `gather_health_report` JSON (`SCOTTY_RLM_HEALTH=1`). Not a substitute for Hermes crew. Offline advisory: `scripts/rlm_spock_advisory.py` + `scripts/rlm_kirk_hook.py` (not wired to ROEE). |
| Ship changes to Hostinger VPS | From repo root (Windows): `.\scripts\deploy_vps.ps1` after committing — runs `git push origin main`, SSH `git pull` on `/opt/Regime-Locus-Matrix`, then restarts **active** systemd units from a list (default: `regime-locus-master`, `rlm-master-telegram`, `rlm-telegram`). Override with `-SystemdUnits` or env `VPS_SYSTEMD_UNITS`. If the server has local edits, use `-StashOnVpsBeforePull`. Defaults: `root@2.24.28.77`; override with env `VPS_HOST`, `VPS_USER`, `VPS_REPO`. |
