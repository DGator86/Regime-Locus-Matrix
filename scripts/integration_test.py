...
try:
    from rlm.optimization.base import OptimizationBase, _signal_based_score, align_regime_labels
    ok("rlm.optimization.base (_signal_based_score, align_regime_labels)")
except Exception as e:
    fail(f"rlm.optimization.base: {e}")

try:
    from rlm.notify.pnl_report import calculate_daily_pnl, _format_log_section
...
try:
    ...
    ok(f"Pipeline ran — factors_df: {len(pipeline_result.factors_df)} rows, "
...
if pipeline_result is not None:
    ...
    check("roee_action" in pdf.columns,
          "policy_df has roee_action", "policy_df MISSING roee_action")
    check("roee_size_fraction" in pdf.columns,
          "policy_df has roee_size_fraction", "policy_df MISSING roee_size_fraction")
    check("close" in pdf.columns,
          "policy_df has close (needed for optimizer scoring)", "policy_df MISSING close")
    ...
    if "hmm_state" in pdf.columns:
        ...
        check(len(states) > 1, f"HMM produced {len(states)} distinct states",
    ...
    check("forecast_return" in fdf.columns,
          "forecast_df has forecast_return", "forecast_df MISSING forecast_return")
    check("hmm_confidence" in fdf.columns,
          "forecast_df has hmm_confidence", "forecast_df MISSING hmm_confidence")
...
if opt_result is not None:
    if not opt_result:
        fail("Optimizer returned empty dict — all trials were pruned. "
...
if live_cfg is not None:
    ...
    info(f"Wrote synthetic overlay with known values")
    ...
    try:
        ...
        checks = [
            ...
            ("forecast.direction_neutral_threshold", overlaid.forecast.direction_neutral_threshold, 0.28),
            ("roee.confidence_threshold (from hmm_confidence_threshold)", overlaid.roee.confidence_threshold, 0.72),
        ...
        for label, got, expected in checks:
            check(abs(float(got) - float(expected)) < 1e-9,
                  f"{label} == {expected}",
...
try:
    ...
    check(abs(pipe_ov.config.roee_config.high_vol_kelly_multiplier - 0.52) < 1e-9,
    ...
    check(abs(pipe_ov.config.roee_config.transition_kelly_multiplier - 0.82) < 1e-9,
    ...
    check(abs(pipe_ov.config.roee_config.calm_trend_kelly_multiplier - 1.18) < 1e-9,
    ...
    check(not result_ov.policy_df.empty, "policy_df non-empty with overlay", "policy_df empty with overlay")
except Exception as e:
...
try:
    proc = subprocess.run(
        [
            sys.executable, str(retune_script),
            "--trade-log", str(tmp_log),
            "--lookback", "20",
            "--warn-threshold", "0.40",
    ...
    check("win rate 25.0%" in stdout or "25.0%" in stdout,
          "Output reports 25.0% win rate",
          f"Win rate not reported correctly in stdout")
    check("nightly_hyperparam_opt" in stdout or "run_nightly" in stdout.lower(),
          "Nightly opt trigger fires (dry-run shows command)",
          "Nightly opt trigger did NOT appear in output")
    check("calibrat" in stdout,
...
synthetic_rows = [
    {"timestamp_utc": ts_str, "plan_id": "EOD_META", "symbol": "META",
     "unrealized_pnl": "-150.0", "closed": "1"},
    {"timestamp_utc": ts_str, "plan_id": "EOD_SPY", "symbol": "SPY",
     "unrealized_pnl": "60.0", "closed": "1"},
    {"timestamp_utc": ts_str, "plan_id": "EOD_QQQ", "symbol": "QQQ",
     "unrealized_pnl": "5.0", "closed": "0"},
]
...
try:
    ...
    check("Exits (closed=1): 2" in report_block,
          "Report shows 2 exits",
          f"Expected '2 exits' — got: {report_block[:300]!r}")
    check("1W / 1L" in report_block,
          "Report shows 1W / 1L",
          "Expected '1W / 1L' not found")
    check("Exit payoff" in report_block,
          "Payoff ratio line present",
          "Payoff ratio line MISSING")
    check("Concentration: META" in report_block,
...
try:
    ...
    check(isinstance(report, str) and len(report) > 50,
