...
Steps:
  ...
  2  Generate synthetic trade-plan artifacts (gate_state, plans, walkforward)
  3  FullRLMPipeline: bars_SPY.csv → factors → HMM regime → ROEE policy
  4  Hermes tool handlers (all 4 RLM tools exercised directly)
  5  Scotty: gather_health_report (data_monitor facts)
  6  Spock:  build_trade_and_regime_context (research_analyst facts)
  7  Kirk:   parse_command_decision + save_decision + SystemGate update
  8  Summary
"""
...
os.environ.setdefault("SCOTTY_AUTO_RESTART", "0")   # no systemctl in health check
...
def step(title: str) -> None:
    global _step
    _step += 1
    print(f"\n{HEAD}{'='*64}{RESET}", flush=True)
    print(f"{HEAD}STEP {_step}: {title}{RESET}", flush=True)
    print(f"{HEAD}{'='*64}{RESET}", flush=True)
...
try:
    from rlm.hermes_facts.kirk_command import (
        CommandDecision, parse_command_decision, save_decision, utc_timestamp,
    )
...
try:
    ...
    ok("rlm.hermes_crew.loop — HermesCrewConfig, skill loaders, _run_full_briefing  [crew loop]")
except Exception as e:
...
try:
    ...
    info("hermes-agent (run_agent) — NOT installed (install with: pip install -e '.[hermes]')")
    info("Health check will exercise all Hermes infrastructure without the LLM call")

# ─────────────────────────────────────────────────────────────────────────────
# STEP 2 — Seed synthetic artifacts so Spock and Kirk have data to read
# ─────────────────────────────────────────────────────────────────────────────
...
plans_path.write_text(json.dumps(plans_payload, indent=2), encoding="utf-8")
ok(f"universe_trade_plans.json — 3 active synthetic plans written")

# --- pipeline artifact (latest run) ---
run_artifact = {
...
run_path = artifacts / "runs" / "SYNTH_RUN_001.json"
...
wf_path = processed / "walkforward_summary_SPY.csv"
wf_path.write_text("\n".join(wf_rows), encoding="utf-8")
ok("walkforward_summary_SPY.csv — 3 OOS windows written")

# --- initial gate state ---
gate_path = processed / "gate_state.json"
...
info(f"All synthetic artifacts seeded under {processed}")

# ─────────────────────────────────────────────────────────────────────────────
# STEP 3 — FullRLMPipeline: bars → factors → HMM → ROEE
# ─────────────────────────────────────────────────────────────────────────────
...
try:
    ...
    info(f"Loaded {len(bars)} daily bars from {bars_path.name}  "
...
if pipeline_result is not None:
    ...
    last = pdf.iloc[-1]
    info(f"Latest bar policy — roee_action={last.get('roee_action','?')}  "
         f"size_fraction={last.get('roee_size_fraction', '?'):.4f}  "
         f"hmm_state={last.get('hmm_state','?')}")

    # Write a pipeline artifact from the actual run
    if "hmm_state" in pdf.columns and "hmm_confidence" in fdf.columns:
        ...
        live_path = artifacts / "runs" / "LIVE_RUN_SPY.json"
        live_path.write_text(json.dumps(live_artifact, indent=2), encoding="utf-8")
        ok(f"artifacts/runs/LIVE_RUN_SPY.json — live pipeline artifact persisted")
...
# These are the exact handlers registered in rlm_hermes_tools/register_rlm_tools.py
# We call them directly to verify they work regardless of whether run_agent is installed.
...
try:
    ...
    check(isinstance(health_json, str) and len(health_json) > 50,
...
try:
    ...
    check(isinstance(ctx_text, str) and len(ctx_text) > 20,
          f"rlm_get_trade_and_regime_context — {len(ctx_text)} chars",
...
try:
    ...
    check(isinstance(limits_json, str),
...
info("--- Scotty's raw health facts (from gather_health_report) ---")
...
info("--- Spock's raw market context (from build_trade_and_regime_context) ---")
...
if hermes_available:
    ...
    # Simulate a well-formed commander response so all downstream parsing/persistence is exercised
    kirk_llm_text = (
    ...
    info("Hermes LLM not available — using synthetic commander response (all parse/persist paths exercised)")
...
if kirk_llm_text:
    ...
    check(bool(decision.command),
          f"command parsed: {decision.command}",
          "command is empty")
    check(bool(decision.rationale),
          f"rationale parsed ({len(decision.rationale)} chars)",
          "rationale is empty")

    save_decision(ROOT, decision)
    decisions_path = ROOT / "data" / "artifacts" / "crew_decisions.json"
    check(decisions_path.is_file(),
          f"crew_decisions.json written ({decisions_path})",
    ...
    ok(f"Commander decision persisted — trading_allowed={gate.is_trading_allowed()}")

    info("--- Final Telegram-format briefing ---")
    telegram_msg = decision.to_telegram_message()
    for line in telegram_msg.splitlines():
        info(f"  {line}")
...
check(tools_reg.is_file(),
      f"rlm_hermes_tools/register_rlm_tools.py — present",
...
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{HEAD}{'='*64}{RESET}", flush=True)
print(f"{HEAD}RLM HEALTH CHECK SUMMARY{RESET}", flush=True)
print(f"{HEAD}{'='*64}{RESET}", flush=True)
...
if _failures:
    ...
    print(f"\n  Hermes crew: {'LIVE (LLM connected)' if hermes_available else 'ACTIVATED (infrastructure verified; install hermes-agent for live LLM calls)'}", flush=True)
    sys.exit(0)
