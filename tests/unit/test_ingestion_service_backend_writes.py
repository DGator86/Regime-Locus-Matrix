...
def test_ingestion_csv_and_lake_writes(monkeypatch, tmp_path):
    monkeypatch.setitem(IngestionService._PROVIDERS, "stub", _StubProvider)
    monkeypatch.setattr(pd.DataFrame, "to_parquet", lambda self, path, index=False: Path(path).write_text("ok", encoding="utf-8"))
