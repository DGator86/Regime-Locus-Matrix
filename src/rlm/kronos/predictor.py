...
class RLMKronosPredictor:
    ...
    * ``predict_paths`` returns all *sample_count* independent sample
    ...
    def _prepare_df(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
        """Normalise an RLM bars DataFrame into Kronos-compatible form.

        Returns (ohlcv_df, timestamp_series).
        """
        out = df.copy()
    ...
    def predict_paths(
        ...
        """Predict *sample_count* independent future OHLCV paths.

        Returns ndarray of shape ``(sample_count, pred_len, 6)`` where
        the 6 columns are ``open, high, low, close, volume, amount``.
        If *future_timestamps* is ``None``, synthetic timestamps are
        generated by extending the last observed interval.
        """
        cfg = self._config
    ...
    def predict_mean(
        ...
        """Return the mean OHLCV forecast, shape ``(pred_len, 6)``."""
        if (
