...
def seed_paper_opens_from_active_plans(
    ...
    concurrent ``ibkr_paper_trade_from_plans`` / pipeline reseed cannot revive a
    position the monitor just closed (TP/stop/trail) while the same
    ``universe_trade_plans.json`` actives are still published.
    """
